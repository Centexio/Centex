...
UniValue obfuscation(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <cntxcoinaddress> <amount>\n"
            "cntxcoinaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <cntxcoinaddress> <amount>\n"
            "cntxcoinaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CNTX address");
...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",  (string) CNTX address of current masternode\n"
...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in cntxcoin.conf\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",    (string) Masternode cntxcoin address\n"
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:37323\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:37323\""));
...
UniValue getmasternodeoutputs (const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (COutput& out, possibleCoins) {
        ...
        obj.push_back(Pair("collateral", std::to_string (out.tx->vout[out.i].nValue / COIN)));

        int ageBlocked = 0;
        if(out.tx->vout[out.i].nValue == Params().MasternodeCollateralLimit1() * COIN) { //exactly
            ageBlocked = Params().MasternodeCollateralLimitMinAge1();
        }
        else if(out.tx->vout[out.i].nValue == Params().MasternodeCollateralLimit2() * COIN) {
            ageBlocked = Params().MasternodeCollateralLimitMinAge2();
        }
        else if(out.tx->vout[out.i].nValue == Params().MasternodeCollateralLimit3() * COIN) {
            ageBlocked = Params().MasternodeCollateralLimitMinAge3();
        }
        CBlockIndex* pindex2 = NULL;
        BlockMap::iterator it = mapBlockIndex.find(out.tx->hashBlock);
        if (it != mapBlockIndex.end())
           pindex2 = it->second;
        ageBlocked += pindex2->nHeight;

        obj.push_back(Pair("release block", (int64_t) ageBlocked));
        ret.push_back(obj);
    }
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) cntxcoin address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",  (string)  cntxcoin MN Address\n"
            "      \"level\": n,           (numeric) Masternode level\n"
