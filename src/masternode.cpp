...
{
    if (blockHeight >= 0 ) {
      if (vin_val == Params().MasternodeCollateralLimit1() * COIN) {return 1;}
      if (vin_val == Params().MasternodeCollateralLimit2() * COIN) {return 2;}
      if (vin_val == Params().MasternodeCollateralLimit3() * COIN) {return 3;}
    }
    return 0;
}
...
*/
    ...
    // should be at least not earlier than block when 1000 cntxcoin tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 cntxcoin tx -> 1 confirmation
