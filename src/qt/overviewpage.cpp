...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CNTX)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    ui->pushButton_Website->setStatusTip(tr("Go to CNTX Website"));
    ui->pushButton_Discord->setIcon(QIcon(GUIUtil::getThemeImage(":/icons/discord")));
    ui->pushButton_Discord->setStatusTip(tr("Join CNTX Discord"));
    ui->pushButton_Telegram->setIcon(QIcon(GUIUtil::getThemeImage(":/icons/telegram")));
    ui->pushButton_Telegram->setStatusTip(tr("Join CNTX Telegram"));
    ui->pushButton_Twitter->setIcon(QIcon(GUIUtil::getThemeImage(":/icons/twitter")));
    ui->pushButton_Twitter->setStatusTip(tr("Read CNTX Twitter"));
    ui->pushButton_Exchange->setIcon(QIcon(GUIUtil::getThemeImage(":/icons/exchange")));
    ui->pushButton_Exchange->setStatusTip(tr("Check out Centex exchange"));
...
//    ui->label_cntxcoin4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_cntxcoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("cntxcoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    if (CurrentBlock >= 0) {
        ...
        ui->roi_12->setText(mn1==0 ? " " : QString::number( Params().MasternodeCollateralLimit1()/roi1,'f',1).append(" days"));
        ui->roi_22->setText(mn2==0 ? " " : QString::number( Params().MasternodeCollateralLimit2()/roi2,'f',1).append(" days"));
        ui->roi_32->setText(mn3==0 ? " " : QString::number( Params().MasternodeCollateralLimit3()/roi3,'f',1).append(" days"));
    }
    CAmount tNodesSumm = mn1*Params().MasternodeCollateralLimit1() + mn2*Params().MasternodeCollateralLimit2() + mn3*Params().MasternodeCollateralLimit3();
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText(QString::number(Params().MasternodeCollateralLimit1()) + " CNTX");
      ui->label_mcolat->setText(QString::number(Params().MasternodeCollateralLimit2()) + " CNTX");
      ui->label_fcolat->setText(QString::number(Params().MasternodeCollateralLimit3()) + " CNTX");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardcntxcoin =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardcntxcoin, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEV_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" CNTX"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("https://centex.io", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://discord.gg/CETCHuU", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://t.me/centexio", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Twitter_clicked() {
    QDesktopServices::openUrl(QUrl("https://twitter.com/centexio", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Exchange_clicked() {
    QDesktopServices::openUrl(QUrl("https://centex.io", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Explorer_clicked() {
    //QDesktopServices::openUrl(QUrl("explorer_url", QUrl::TolerantMode));
}
