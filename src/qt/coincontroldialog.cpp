...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useSwiftTX) nPayFee = max(nPayFee, MIN_SWIFTTX_FEE);
        // Allow free?
    ...
    int nDisplayUnit = BitcoinUnits::CNTX;
	...
	for(const COutput& out: coins.second) {
		...
		if (fMultisigEnabled && !fMultiSigUTXO)
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show cntxcoin address. In tree mode, address is not shown again for direct wallet address outputs
