...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("cntxcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("cntxcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CNTX Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cntx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            if (Params().NetworkID() == CBaseChainParams::MAIN)
                while (chainActive.Tip()->nTime < 1471482000 || vNodes.size() <= 2 || !masternodeSync.IsSynced() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance()) {
        ...
        //LogPrintf("Running cntxcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
